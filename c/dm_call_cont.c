...
void initialize_from_input(double *w_in, double alpha0, double eta0, double tau20, double *w,
...
/*	int *y_int;*/
	...
	if (!isReal(initials) || !isVector(initials))
		error("[ERROR] initial parameter argument should be a real vector\n");
	else {
...
/**
 * w is initialized from input in R, most likely from last MCMC run
