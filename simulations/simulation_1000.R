...
simulate_y_poisson<-function(n, sub_neighbor,alpha,eta,tau2,M){
  #Simulate w's
  #using Gibbs
  w = rep(0,n)
  for (t in 1:M){
...
simulate_y_poisson_ms<-function(n, sub_neighbor,alpha,eta,tau2,m,M){
  #updated 20180831
  #simulate multiple samples of y (techinical replicates)
  #m is number of samples
  #M is number of simulation iterations
  #Simulate w's
  #using Gibbs
  w = rep(0,n)
  for (t in 1:M){
...
simulate_y_poisson_mvn<-function(n, sub_neighbor, alpha, eta, tau2, m) {
  #updated 20180906
  #simulated multiple samples of y (technical replicates)
  #from the multivariate normal distribution (marginal, not conditional independent)
  w = rep(0,n)
  ...
  if (eta==0) {
    ...
    diag(I) <-1
    temp <- I - eta*sub_neighbor
    inv <-chol2inv(chol(temp))
    Sigma = inv%*%M
  }
